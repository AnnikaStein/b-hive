...
class BetterParticleTransformer(nn.Module):
    ...
    def __init__(self,
                 ...
                 feature_edges = None,
                 **kwargs):
        super(BetterParticleTransformer, self).__init__(**kwargs)
        ...
        if self.small:
            ...
            self.cls_token = nn.Parameter(torch.zeros(1, 8, embed_dim), requires_grad=True)
    ...
    def forward(self, x):
        ...
        if self.small:
            cpf, cpf_4v = cpf[:,:25,:], cpf_4v[:,:25,:]
            cls_4v = torch.zeros(cpf_4v.shape[0], 1, 4).to(cpf.device)
        else:
            cls_4v = torch.zeros(cpf_4v.shape[0], 8, 4).to(cpf.device)

        padding_mask = torch.cat((cls_4v[:,:,:1],cpf_4v[:,:,:1],npf_4v[:,:,:1],vtx_4v[:,:,:1]), dim = 1)
        ...
        enc = torch.cat((cls_tokens,enc),dim = 1)

        lorentz_vectors = torch.cat((cls_4v,cpf_4v,npf_4v,vtx_4v), dim = 1)
